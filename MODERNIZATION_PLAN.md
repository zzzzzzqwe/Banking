...
1. Savings Goals (Vaults/Копилки)                                                                                                                                                                                          
   Пользователь создаёт цель (например "Отпуск" — 3000 EUR), привязывает к счёту, вносит деньги. Красивый progress bar с процентами, анимация при достижении цели. Revolut-style фича.

2. Recurring Payments (Автоплатежи)                        
   Настраиваемые регулярные переводы (раз в неделю/месяц). Scheduler на бэкенде через @Scheduled. Таблица с ближайшими платежами, toggle вкл/выкл.

3. Card Freeze / Unfreeze
   Быстрый toggle прямо на карте — заморозить/разморозить. Визуально карта становится серой с эффектом "льда". Уже есть AccountStatus и admin freeze — нужно просто дать юзеру self-service.

4. Subscription Tracker
   Автодетект повторяющихся списаний (одинаковая сумма + категория + интервал). Отдельная страница со списком подписок, суммой в месяц, возможностью пометить/отменить.

5. Activity Feed / Notifications Center
   Колокольчик в хедере с real-time уведомлениями (перевод получен, кредит одобрен, цель достигнута). Dropdown с историей. У тебя уже есть event system на бэке — нужно только SSE/WebSocket + фронт.

6. Split Bill
   Разделить транзакцию с другим пользователем — выбираешь сумму, юзера, отправляешь запрос. Тот видит в notifications и подтверждает.